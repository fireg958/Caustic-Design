...
void Mesh::exportVertices(const QString& filename, float scaling){
    ...
    std::cout<<"[exporting] "<<std::endl<<"CAUSTIC_DOMAIN="<<CAUSTIC_DOMAIN<<" "<<"scaling="<<scaling<<std::endl;
    std::cout<<"verticesMesh.size="<<verticesMesh.size()<<endl;
...
int Mesh::getEdgeIndex(int i)
{
    if(allToEdgeMapping[i] == -1){
        std::cerr<<"vertex index"<<i<<"is not an edge"<<std::endl;
        return-1;
    }
    return allToEdgeMapping[i]; //return the index of the vertex in vertices in the faceVerticesEdge
